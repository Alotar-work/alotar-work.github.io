document.addEventListener("DOMContentLoaded", function () {
  ...
  if (keychainClear == 1) {
    ...
    keychainClear = 0;
    debugConsole.log("Очищеный куки");
  }

  var keychainDesign = getCookie("keychainDesign");
  if (keychainDesign) {
    keychainDesign = JSON.parse(keychainDesign);
    debugConsole.log("Получены данные о брелоках из кук:");
    debugConsole.log(keychainDesign);
  } else {
    keychainDesign = [];
    debugConsole.log("Данные о брелоках не найдены в куках");
  }
  ...
  function handleSelectChange(optionText) {
    ...
    debugConsole.log("Измененно количество элементов");
  }

  const selectCheck = setInterval(() => {
    const $selectElement = $("select.js--selectize");
    debugConsole.log("Ищем select с выбором количества элементов");
    if ($selectElement.length) {
      debugConsole.log("Проверяем, инициализирован ли selectize");
      if ($selectElement[0].selectize) {
        debugConsole.log("Selectize инициализирован");
        ...
        debugConsole.log("Selectize обработчик установлен");
      } else {
        debugConsole.log(
        ...
        if (hasOptions) {
          debugConsole.log("Опции найдены");
          ...
          clearInterval(selectCheck);
          debugConsole.log("Обычный select обработчик установлен");
        }
      }
    }
  }, 1000);
  ...
  if (addBtn) {
    debugConsole.log("Найдено кнопка добавления в корзину");
    const container = addBtn.parentNode;

    addBtn.style.position = "absolute";
    addBtn.style.top = "-999px";
    debugConsole.log("Кнопка добавления в корзину скрыта");
    ...
    debugConsole.log("Добавлено поле 'Email'");
    ...
    doneBtn.addEventListener("click", function () {
      ...
      debugConsole.log("Получены данные конструктора:");
      ...
      debugConsole.log("Данные брелоков перед изменением:");
      debugConsole.log(keychainDesign);
      var fourCount = 0;
      var eightCount = 0;
      ...
      debugConsole.log("Уже добалено в корзину:");
      debugConsole.log("Брелоков с четырмя элементами - " + fourCount);
      debugConsole.log("Брелоков с восьмью элементами - " + eightCount);
      if (
        fourCount == 3 ||
      ...
      if (resultObj.elements.length == 4 || resultObj.elements.length == 8) {
        ...
        debugConsole.log("Данные из кук:");
